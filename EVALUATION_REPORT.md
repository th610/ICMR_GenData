...
- 이후 4-6턴의 위반 대화 자동 생성
- 장점: 현실적 맥락, 자연스러운 전개

**방법 2: Normal 라벨 데이터 확보**
- ESConv 전체 1,300개 대화를 LLM-Judge로 평가
- 위반 없는 대화를 Normal 라벨로 활용
- 평가 결과: 1,300개 중 **1,287개가 Normal** (99.0%)

### 1.3 ESConv LLM-Judge 평가 결과
```
총 평가: 1,300개 대화
- Normal: 1,287개 (99.0%)
- V1 (맥락/정보수집 실패): 8개 (0.6%)
- V2 (지시/강요): 3개 (0.2%)
- V3 (공감 실패): 2개 (0.2%)
- V4/V5: 0개

→ ESConv는 고품질 상담 대화 (위반율 1.0%)
```

**활용 의의:**
- ✅ Normal 라벨의 풍부한 데이터 확보 (1,287개)
- ✅ 실제 상담 대화 기반의 현실적 맥락
- ✅ 위반 케이스 생성을 위한 prefix 자원
...
| 라벨 | 생성 방법 | 개수 (목표) | 설명 |
|------|----------|------------|------|
| Normal | ESConv 원본 | 1,287개 | LLM-Judge 검증된 정상 대화 |
| V1 | ESConv Prefix + Insertion | 300개 | 맥락 이해 / 정보수집 실패 |
| V2 | ESConv Prefix + Insertion | 300개 | 지시 / 강요 |
| V3 | ESConv Prefix + Insertion | 300개 | 공감 실패 |
| V4 | Full Generation | 150개 | 비현실적 신념 제시 |
| V5 | Full Generation | 150개 | 위기 상황 비전문적 대응 |
| **합계** | - | **2,487개** | - |

### 2.2 생성 방식별 설명

#### A. Prefix + Insertion 방식 (V1, V2, V3)
...
### 3.2 ESConv 전체 평가 결과 (1,300개)

| 라벨 | 개수 | 비율 | 설명 |
|------|------|------|------|
| Normal | 1,287 | 99.0% | 적절한 상담 |
| V1 | 8 | 0.6% | 맥락 이해 실패 |
| V2 | 3 | 0.2% | 지시적 언어 |
| V3 | 2 | 0.2% | 공감 부족 |
| V4 | 0 | 0.0% | - |
...
**발견사항:**
- ✅ ESConv는 매우 고품질 데이터 (99% Normal)
- ✅ 실제 사람이 작성한 대화는 위반율 낮음
- ✅ V4/V5 같은 극단적 위반은 거의 없음
- ⚠️ 따라서 V4/V5는 별도 생성 필요

### 3.3 생성 데이터 LLM-Judge 검증 결과

#### V1, V2, V3 검증 (각 300개 생성 → 일부 샘플 검증)
```
검증 샘플: 각 라벨당 20개씩 무작위 추출
...
**품질 분석:**
- ✅ V2, V4, V5: 매우 높은 정확도 (100%)
- ⚠️ V1: 가끔 V2와 혼동 (지시적 표현 포함 시)
- ⚠️ V3: 가끔 Normal로 오판 (공감 표현이 애매한 경우)
...
Normal          0.89     0.92     0.90      257
V1              0.82     0.79     0.80       60
V2              0.87     0.85     0.86       60
V3              0.84     0.81     0.82       60
V4              0.91     0.94     0.92       30
V5              0.89     0.91     0.90       30

Macro Avg       0.87     0.87     0.87      497
Weighted Avg    0.87     0.87     0.87      497
```

**Confusion Matrix 주요 발견:**
- Normal ↔ V1: 가장 많은 혼동 (맥락 이해가 미묘한 경우)
- V1 ↔ V2: 일부 혼동 (지시적 표현이 포함된 V1)
- V4, V5: 매우 높은 정확도 (극단적 위반이라 명확함)
...
3. **V4/V5 데이터 부족**
   - 각 150개씩만 생성
   - 실제 사례 거의 없음 (ESConv/EmpatheticDialogues에서 0건)
   - 모델 학습 데이터 부족
...
- [ ] 사용자는 Agent의 응답을 실제 상담사와 구별할 수 있는가?
...
### 8.1 달성한 것
✅ ESConv 기반 2,487개 고품질 데이터셋 구축
✅ LLM-Judge 시스템으로 95%+ 정확도 검증
✅ RoBERTa 분류 모델 86.8% 정확도 달성
✅ Agent 아키텍처 설계 및 타당성 검증 (5% 위반율)
✅ 외부 데이터셋(EmpatheticDialogues) 검증 완료
✅ Critical bug 발견 및 수정 (tokenizer 이슈)

### 8.2 핵심 인사이트
💡 ESConv는 매우 고품질 (99% Normal)
💡 실제 대화는 위반율 낮음 (5%) → Agent 아키텍처 타당
💡 Synthetic extreme data는 real-world와 분포 불일치
💡 Judge API > 분류 모델 (맥락 이해, 설명 가능성)
💡 Special token, 대규모 데이터, Hybrid 접근이 필요

### 8.3 다음 스텝
1. Special token 추가 재학습 (1주)
2. EmpatheticDialogues API 라벨링 (2주)
3. Hybrid Judge 시스템 구현 (1주)
4. Agent 프로토타입 구축 (2주)
5. 파일럿 테스트 (4주)
