...
from scripts.pilot.judge_quality import build_judge_prompt, JUDGE_SYSTEM
...
def judge_session(session: dict, llm_client: OpenAIClient):
    """개별 세션 Judge 평가 (대화 내용 포함)"""
    
    session_id = session.get('session_id', 'unknown')
    
    try:
        # expected_label을 "Unknown"으로 (원본이므로)
        user_prompt = build_judge_prompt(session, "Unknown", llm_client)
