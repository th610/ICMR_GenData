...
def evaluate_empathetic(max_samples: int = 100,
    ...
    for conv_id, conv_data in list(conversations.items())[:max_samples]:
        ...
        # Build dialog - first speaker is seeker, alternating
        dialog = []
        first_speaker_id = utterances[0]['speaker_idx']
        for utt in utterances:
            speaker = "seeker" if utt['speaker_idx'] == first_speaker_id else "supporter"
            dialog.append({
                "speaker": speaker,
                "text": utt['utterance']
            })
